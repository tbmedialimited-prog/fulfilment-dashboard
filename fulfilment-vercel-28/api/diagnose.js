...
module.exports = async (req, res) => {
  ...
  // Status ID breakdown for today's orders
  try {
    ...
    results.status_breakdown = statusCounts;
    results.status_id_meanings = 'Check GET /api/Order/Statuses for names';

    // Also fetch the actual status names
    const sr = await client.get('/Order/Statuses');
    results.all_statuses = Array.isArray(sr.data) ? sr.data.map(s=>({id:s.ID,name:s.Name})) : sr.data;
  } catch(e) { results.status_breakdown = { error: e.message }; }
